...
@section('page-title', 'Point Rules')
@section('page-sub', 'Kelola aturan poin dinamis karyawan')
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-weight: 600; color: var(--white);">{{ $rule->rule_name }}</td>
                    <td>
                        @if($rule->condition_type === 'check_in')
                            <span class="badge badge-blue">Jam Masuk</span>
                        @elseif($rule->condition_type === 'late_minutes')
                            <span class="badge badge-orange">Menit Telat</span>
                        @else
                            <span class="badge badge-red">Alfa</span>
                        @endif
                    </td>
                    <td style="font-family: var(--mono);">{{ $rule->condition_operator }}</td>
                    ...
                    <td>
                        @if($rule->point_modifier > 0)
                            <span class="badge badge-green">+{{ $rule->point_modifier }} poin</span>
                        @else
                            <span class="badge badge-red">{{ $rule->point_modifier }} poin</span>
                        @endif
                    </td>
                    <td>
                        <form method="POST" action="{{ route('admin.point-rules.toggle', $rule) }}">
                            @csrf @method('PATCH')
                            <button type="submit" class="badge {{ $rule->is_active ? 'badge-green' : 'badge-red' }}" style="border:none;cursor:pointer;">
                    ...
                    <td>
                        <div style="display: flex; gap: 6px;">
                            <button onclick="openEditModal({{ $rule->id }}, '{{ $rule->rule_name }}', '{{ $rule->condition_type }}', '{{ $rule->condition_operator }}', '{{ $rule->condition_value }}', '{{ $rule->condition_value_max }}', {{ $rule->point_modifier }})"
                                class="btn btn-secondary btn-sm">Edit</button>
                            <form method="POST" action="{{ route('admin.point-rules.destroy', $rule) }}"
...
<div id="modalTambah" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.6); z-index:999; align-items:center; justify-content:center;">
    <div class="card" style="width:100%; max-width:480px; margin:0 16px;">
        ...
        <form method="POST" action="{{ route('admin.point-rules.store') }}" style="padding: 20px; display:flex; flex-direction:column; gap:14px;">
            @csrf
            <div>
...
function openEditModal(id, name, type, operator, value, valueMax, modifier) {
    const base = "{{ url('admin/point-rules') }}/" + id;
