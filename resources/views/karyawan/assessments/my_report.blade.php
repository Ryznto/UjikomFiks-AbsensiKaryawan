...
@section('content')
    ...
    <form method="GET" style="margin-bottom:16px;">
        <input type="month" name="period_month" class="form-control" value="{{ now()->format('Y-m') }}"
    ...
    @if ($currentAssessment)
        <div class="card fade-in" style="margin-bottom:16px; text-align:center;">
            <div class="card-body" style="padding:24px;">
                <div style="font-size:12px; color:var(--mid); margin-bottom:8px;">Rata-rata Nilai – {{ $period }}
                </div>
                <div
                ...
                <div style="margin-top:12px;">
                    <span class="badge badge-{{ $currentAssessment->score_badge }}"
                    ...
                    <div class="accordion-item" style="border:1px solid #e5e7eb; border-radius:12px; overflow:hidden;">
                        <div class="accordion-header" onclick="toggleAccordion({{ $loop->index }})"
                            ...
                            <div style="display:flex; align-items:center; gap:10px;">
                                ...
                                <span
                                    style="font-weight:700; color:#4f7cff; font-size:0.9rem;">{{ $avgCat }}/5</span>
                            ...
                            @foreach ($details as $detail)
                                <div>
                                    <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
                                        ...
                                        <span
                                            style="font-weight:700; color:#4f7cff; font-size:0.85rem; white-space:nowrap; margin-left:8px;">{{ number_format($detail->score, 1) }}/5</span>
                                    </div>
                                    <div>
                                        @for ($i = 1; $i <= 5; $i++)
                                            <span
                                                style="font-size:0.9rem; color:{{ $i <= $detail->score ? '#fbbf24' : '#e5e7eb' }}">★</span>
                                        @endfor
                                    </div>
                                    @php
                                    ...
                                    <div style="background:#f0f0f0; border-radius:99px; height:6px; margin-top:4px;">
                                        <div
                                            style="width:{{ $pct }}%; background:{{ $color }}; height:100%; border-radius:99px;">
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                ...
                @if ($currentAssessment->general_notes)
                    <div
                        ...
                        <p style="margin:0; line-height:1.7; font-size:0.9rem; color:rgba(255,255,255,0.7);">
                            {{ $currentAssessment->general_notes }}</p>
                    </div>
                @endif
            </div>
        </div>
